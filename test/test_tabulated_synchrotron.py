...
def main(bfield):
    ...
    sp0 = SpectrumParametrization(n0=1e34 / u.eV, e0=1e12 * u.eV,
    ...
    lum_conv = np.trapezoid(table.T * sp0.dn_de0(electron_energy), electron_energy, axis=1)
    t2 = time.time()
    print(f"furst in {t1-t0:.3f} seconds, second in {t2-t1:.3f} seconds")

    set_plotting_defaults()
    plt.plot(photon_energy, (photon_energy * lum_straight).to(u.erg/u.s))
    plt.plot(photon_energy, (photon_energy * lum_conv).to(u.erg/u.s), linestyle='dashed')
    plt.xscale('log')
    plt.yscale('log')
    plt.show()
