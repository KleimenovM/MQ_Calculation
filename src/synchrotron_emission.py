...
def single_electron_synchrotron_emission_power_symmetrized(electron_energy, photon_energy, bfield):
    alpha = np.linspace(1e-2, np.pi/2, 20)  # angle variables
    ph_a, a_ph = np.meshgrid(photon_energy, alpha, indexing='ij')
    # e_c is actually a function of angle
    e_c = CST_HC / (4 * np.pi) * 3 * CST_e * bfield * (electron_energy ** 2 - CST_m_e ** 2) / CST_m_e**3 * np.sin(a_ph)
    x = (ph_a / e_c).to('')  # [DL]
    f_x = first_synchrotron_function_approximation(x)

    # P_emitted = (dim_factor * f(x)) * sin(alpha)
    # P_received = P_emitted / sin^2(alpha) = (dim_factor / sin(alpha))
    # Now we average over the sphere -> integral (... * sin(alpha) d(alpha)) / integral(sin(alpha) d(alpha))
    # The lower integral == 1, hence we need just to calculate average over alpha

    dim_factor = np.sqrt(3) * CST_e**3 * bfield / (cst.h * CST_m_e)
    return np.average(dim_factor * f_x, axis=1).to(1/u.s)
...
def single_electron_synchrotron_emission_power(electron_energy, photon_energy, bfield):
    ...
    p_dl = first_synchrotron_function_approximation(x)
    dim_factor = np.sqrt(3) * CST_e**3 * bfield / (cst.h * CST_m_e)
...
def electron_synchrotron_emission_luminosity(electron_energy, electron_density,
    ...
    :param electron_density: [eV-1], elelectron_synchrotron_emission_luminosityectron number per energy bin.
